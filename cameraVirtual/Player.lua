-- Chamamos o script do teclado para o player
local scriptTeclado = require ("Teclado")

local Player = {}
-- hud está ligado a variável criada no main que executa a função .novo no scripto HUD
function Player.novo (x, y, hud)
    ...
    -- Ativando a função global .novo do scripto do teclado.
    scriptTeclado.novo (player)
    -- "self" seria a colisão principal no caso o "Player"
    ...
    return player -- referente a variável player, ao objeto, ao personagem
end
return Player -- referente ao script player (return significa PARAR a função)