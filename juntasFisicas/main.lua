...
-- bodies [#bodies+1] = shape1 -- Adiciona o shape na tabela/string bodies sem sobrescrever.
...
-- -- Início da função: (self trata do objeto ouvinte)
...
--                 function ()
--             -- Cria-se a junta já dentro da string joints, onde o objA é o colidido e o B é o ouvinte da função, e as âncoras são criadas de acordo com a localização do objB.    
--                     joints[#joints+1]= physics.newJoint("friction", event.other, self, self.x, self.y) -- event.other (objetoA), self (objeto que foi colidido, o ouvinte). Altera os valores máximos de força e torque de acordo com o obtido no if acima.
...
-- bumperB.x, bumperB.y = staticBox.x+staticBox.width/2-15, 385  -- staticBox.x (localização do objeto) + staticBox.width (largura do objeto) / 2 - 15
...
-- -- Aplicação de torque as rodas para movimentação do carrinho
...
-- Criando a junta ("tipo de junta", corpoA, corpoB, pinoA.x, pinoA.y [ponto fixo 1], pinoB.x, pinoB.y [ponto fixo 2], ancoraA.x, ancoraA.y [tamanho da corda 1], ancoraB.x, ancoraB.y [tamanho da corda 2], relação da polia/força da corda) a relação da polia em 1.0 é como funciona real
