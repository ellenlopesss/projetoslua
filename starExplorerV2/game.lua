...
local function criarAsteroide ()
    ...
    -- Incluindo o asteroide que foi criado na tabela.
...
local function atirar ()
    ...
    local laser = display.newImageRect (mainGroup, sprite, 5, 15, 40)
...
local function moverNave (event)
    ...
    elseif ("ended" == pahse or "cancelled" == phase) then
...
local function gameLoop ()
    criarAsteroide ()
-- grupo (ex. #asteroidesTable = 10 e , 1 inicio e -1  menus um ate 1)
...
-- create() scene:create() - Ocorre quando a cena é criada pela primeira vez, mas ainda não apareceu na tela.
function scene:create( event )
	...
	nave = display.newImageRect (mainGroup, sprite, 4, 98, 97)
	nave.x = display.contentCenterX
	nave.y = display.contentHeight - 100
	physics.addBody (nave, "dynamic", {radius=30, isSensor=true}) -- isSensor: quando utiliza o "issensor" quer dizer que o corpo fisico é um sensor, detecta as colisões que os osrpos colidem com ele, mas ele não tem nenhum tipo de retorno físico. O Bullet é a bala, o tiro, é quando ele vai estar com detecção de colisão contínua, utiliza quando vai fazer contato entre os objetos que valha pontuação por exemplo.
	nave.myName = "Nave"
...
-- show() scene:show() - Ocorre imediatamente antes e/ou imediatamente após a cena aparecer na tela. 
function scene:show( event )
	...
	elseif ( phase == "did" ) then
		...
		physics.start()
		gameLoopTimer = timer.performWithDelay (700, gameLoop, 0) -- a cada 700 milisegundos executa a fução gameLoop
...
-- hide() scene:hide() - Ocorre imediatamente antes e/ou imediatamente após a cena sair da tela. 
...
-- destroy() scene:destroy () - Ocorre quando a cena é destruída.
